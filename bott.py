...
@bot.message_handler(func=lambda message: True, commands=['start'])# начать работу с команды start
def Start(message):
    try:
...
def info(message):
    bot.send_message(message.chat.id, "Welcome to the investing bot.")
    bot.send_message(message.chat.id, "intervals allowed: 1min, 5min, 15min, 30min, 60min, daily, weekly, monthly. ")
